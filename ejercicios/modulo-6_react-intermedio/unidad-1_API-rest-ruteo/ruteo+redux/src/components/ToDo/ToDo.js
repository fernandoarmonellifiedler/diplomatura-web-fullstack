...
class ToDo extends React.Component {
  ...
  async addItem(_name, _isCompleted) {
    ...
    if (respuesta.status === 201) {
      // ok => Agregar la tarea al listado
...
const mapActionsToProps = (dispatch) => {
  return {
    onRefresh: (nuevoListado) => {
      dispatch({ type: 'REFRESH', data: nuevoListado });
    },
    onAdd: (nuevoElemento) => {
      dispatch({ type: 'ADD', data: nuevoElemento });
    },
    onRemove: (id) => {
      dispatch({ type: 'REMOVE', data: id });
    },
    onUpdate: (elementoActualizado) => {
      dispatch({ type: 'UPDATE', data: elementoActualizado });
    },
  };
};
